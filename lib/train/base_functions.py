...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "GOT10K_official_val", "COCO17", "VID", "TRACKINGNET",
                        "DepthTrack_train", "DepthTrack_val", "LasHeR_all", "LasHeR_train", "LasHeR_val", "VTUAV", "VTUAVST", "VisEvent"]
        ...
        if name == "VTUAV":
            datasets.append(VTUAV(settings.env.vtuav_dir, split='train'))
        if name == "VTUAVST":
            datasets.append(VTUAV(settings.env.vtuav_dir, split='val_st'))
        if name == "VisEvent":
...
def get_optimizer_scheduler(net, cfg):
    ...
    addtion_param = ['decode', 'CDA', 'CSS', 'MCI', 'moe']
    param_dicts = [
