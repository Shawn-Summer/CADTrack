...
from lib.test.tracker.cadtrack import CADTrack
import lib.test.parameter.cadtrack as rgbt_params
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=300, debug=0, script_name='prompt'):
    ...
    if script_name == 'cadtrack':
        params = rgbt_params.parameters(yaml_name, epoch)
        cadtrack = CADTrack(params)
        tracker = CADTRACK_RGBT(tracker=cadtrack)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--script_name', type=str, default='cadtrack', help='Name of tracking method(ostrack, prompt, ftuning).')
    parser.add_argument('--yaml_name', type=str, default='cadtrack_lasher', help='Name of tracking method.')  # vitb_256_mae_ce_32x4_ep300 vitb_256_mae_ce_32x4_ep60_prompt_i32v21_onlylasher_rgbt
    parser.add_argument('--dataset_name', type=str, default='LasHeR', help='Name of dataset (GTOT,RGBT210,RGBT234,LasHeR,VTUAVST,VTUAVLT).')
    ...
    if dataset_name == 'GTOT':
        seq_home = '/home/sqh/lihao/STTrack/data/RGBTdatasets/GTOT'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
    elif dataset_name == 'RGBT210':
        seq_home = '/home/sqh/lihao/STTrack/data/RGBTdatasets/RGBT210'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
    elif dataset_name == 'RGBT234':
        seq_home = '/home/sqh/lihao/STTrack/data/RGBTdatasets/RGBT234'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
    elif dataset_name == 'LasHeR':
        seq_home = '/home/sqh/lihao/STTrack/data/RGBTdatasets/LasHeR/test'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
    elif dataset_name == 'VTUAVST':
        seq_home = '/home/sqh/lihao/STTrack/data/RGBTdatasets/VTUAV'
